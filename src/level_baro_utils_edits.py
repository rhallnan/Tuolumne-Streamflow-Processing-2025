...
def plot_in_water(df, insert_datetime_str, remove_datetime_str, site_info, sensor_type='lvl', figsize=config.FIGSIZE):
    ...
    #insert_start = max(df.index.min(), insert_datetime - pd.Timedelta(hours=12))
    # Convert input datetimes
    insert_datetime = pd.to_datetime(insert_datetime_str)
    remove_datetime = pd.to_datetime(remove_datetime_str)
    #insert_datetime = datetime.datetime.strptime(insert_datetime_str, '%Y-%m-%d %H:%M')
    #insert_start = max(df.index[0], insert_datetime - timedelta(hours=12))
    #insert_start = max(df.index.min(), insert_datetime - pd.Timedelta(hours=12))
    #insert_stop = insert_datetime + pd.Timedelta(hours=12)
    ...
    ax[0].plot(df[sensor_type + '_cm'][insert_datetime:remove_datetime], label='Full Series')
    ...
    ax[1].plot(df['temp_C'][insert_datetime:remove_datetime], label='Full Series')
    ...
    remove_stop = min(df.index[-1], remove_datetime + pd.Timedelta(hours=12))
    remove_start = remove_datetime - pd.Timedelta(hours=12)
...
def read_vented_segments(sitecode):
    ...
    for fn in os.listdir(vented_dir_path):
        if config.SITE_SHORTNAME[sitecode] in fn:
            ...
            vented_segments.append(pd.read_csv(data_path, index_col=0))
